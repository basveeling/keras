...
from keras.backend import pytorch_backend as PTC, floatx, set_floatx, variable
from keras.backend import theano_backend as KTF
...
def check_single_tensor_operation(function_name, input_shape, **kwargs):
    ...
    xth = PTC.variable(val)
    xtf = KTF.variable(val)

    zth = PTC.eval(getattr(PTC, function_name)(xth, **kwargs))
...
def check_two_tensor_operation(function_name, x_input_shape,
    ...
    xth = PTC.variable(xval)
    xtf = KTF.variable(xval)

    yval = np.random.random(y_input_shape) - 0.5

    yth = PTC.variable(yval)
    ytf = KTF.variable(yval)

    zth = PTC.eval(getattr(PTC, function_name)(xth, yth, **kwargs))
...
def check_composed_tensor_operations(first_function_name, first_function_args,
    ...
    yth = getattr(PTC, first_function_name)(xth, **first_function_args)
    ytf = getattr(KTF, first_function_name)(xtf, **first_function_args)

    zth = PTC.eval(getattr(PTC, second_function_name)(yth, **second_function_args))
...
class TestPyTorchBackend(object):
    ...
    def test_shape_operations(self):
        ...
        yth = PTC.variable(yval)
        ytf = KTF.variable(yval)
        zth = PTC.eval(PTC.concatenate([xth, yth], axis=-1))
    ...
    def test_repeat_elements(self):
        ...
        for ndims in [1, 2, 3]:
            ...
            arr_th = PTC.variable(arr)
            arr_tf = KTF.variable(arr)

            for rep_axis in range(ndims):
                np_rep = np.repeat(arr, reps, axis=rep_axis)
                th_rep = PTC.eval(
                    PTC.repeat_elements(arr_th, reps, axis=rep_axis))
                tf_rep = KTF.eval(
    ...
    def test_tile(self):
        ...
        n = (2, 1)
        th_rep = PTC.eval(PTC.tile(arr_th, n))
    ...
    def test_value_manipulation(self):
        ...
        # count_params
        # TODO: uncomment
        # assert PTC.count_params(xth) == KTF.count_params(xtf)
        ...
        xth = PTC.variable(val)
        xtf = KTF.variable(val)
        assert PTC.get_variable_shape(xth) == KTF.get_variable_shape(xtf)
    ...
    def test_gradient(self):
        ...
        expth = xth * PTC.exp(xth)
        exptf = xtf * KTF.exp(xtf)
        lossth = PTC.sum(expth)
        losstf = KTF.sum(exptf)
        zero_lossth = PTC.stop_gradient(lossth)
        zero_losstf = KTF.stop_gradient(losstf)

        gradth = PTC.gradients(lossth, [expth])
        gradtf = KTF.gradients(losstf, [exptf])
        zero_gradth = PTC.gradients(lossth + zero_lossth, [expth])
        zero_gradtf = KTF.gradients(losstf + zero_losstf, [exptf])

        zth = PTC.eval(gradth[0])
        ztf = KTF.eval(gradtf[0])
        zero_zth = PTC.eval(zero_gradth[0])
    ...
    def test_function(self):
        ...
        xth = PTC.variable(val)
        xtf = KTF.variable(val)
        yth = PTC.placeholder(ndim=2)
        ytf = KTF.placeholder(ndim=2)

        exp_th = PTC.square(xth) + yth
        exp_tf = KTF.square(xtf) + ytf

        update_th = xth * 2
        update_tf = xtf * 2
        fth = PTC.function([yth], [exp_th], updates=[(xth, update_th)])
    ...
    def test_rnn(self):
        ...
        # test default setup
        th_rnn_step_fn = rnn_step_fn(input_dim, output_dim, PTC)
        th_inputs = PTC.variable(input_val)
        th_initial_states = [PTC.variable(init_state_val)]
        last_output, outputs, new_states = PTC.rnn(th_rnn_step_fn, th_inputs,
                                                   th_initial_states,
                                                   go_backwards=False,
                                                   mask=None)
        th_last_output = PTC.eval(last_output)
        th_outputs = PTC.eval(outputs)
        assert len(new_states) == 1
        th_state = PTC.eval(new_states[0])
        ...
        # test unroll
        unrolled_last_output, unrolled_outputs, unrolled_new_states = PTC.rnn(
        ...
        unrolled_th_last_output = PTC.eval(unrolled_last_output)
        unrolled_th_outputs = PTC.eval(unrolled_outputs)
        assert len(unrolled_new_states) == 1
        unrolled_th_state = PTC.eval(unrolled_new_states[0])
        ...
        bwd_last_output, bwd_outputs, bwd_new_states = PTC.rnn(
            th_rnn_step_fn, th_inputs,
            th_initial_states,
            go_backwards=True,
            mask=None)
        bwd_th_last_output = PTC.eval(bwd_last_output)
        bwd_th_outputs = PTC.eval(bwd_outputs)
        assert len(bwd_new_states) == 1
        bwd_th_state = PTC.eval(bwd_new_states[0])

        bwd_unrolled_last_output, bwd_unrolled_outputs, bwd_unrolled_new_states = PTC.rnn(
            th_rnn_step_fn, th_inputs,
            th_initial_states,
            go_backwards=True,
            mask=None,
        ...
        bwd_unrolled_th_last_output = PTC.eval(bwd_unrolled_last_output)
        bwd_unrolled_th_outputs = PTC.eval(bwd_unrolled_outputs)
        assert len(bwd_unrolled_new_states) == 1
        bwd_unrolled_th_state = PTC.eval(bwd_unrolled_new_states[0])
        ...
        th_mask = PTC.variable(np_mask)

        masked_last_output, masked_outputs, masked_new_states = PTC.rnn(
            th_rnn_step_fn, th_inputs,
            th_initial_states,
            go_backwards=False,
            mask=th_mask)
        masked_th_last_output = PTC.eval(masked_last_output)
        masked_th_outputs = PTC.eval(masked_outputs)
        assert len(masked_new_states) == 1
        masked_th_state = PTC.eval(masked_new_states[0])

        unrolled_masked_last_output, unrolled_masked_outputs, unrolled_masked_new_states = PTC.rnn(
            th_rnn_step_fn, th_inputs,
            th_initial_states,
            go_backwards=False,
            mask=th_mask,
            unroll=True,
            input_length=timesteps)
        unrolled_masked_th_last_output = PTC.eval(unrolled_masked_last_output)
        unrolled_masked_th_outputs = PTC.eval(unrolled_masked_outputs)
        assert len(unrolled_masked_new_states) == 1
        unrolled_masked_th_state = PTC.eval(unrolled_masked_new_states[0])
    ...
    def test_switch(self):
        ...
        xth = PTC.variable(val)
        xth = PTC.switch(xth >= 0.5, xth * 0.1, xth * 0.2)
    ...
    def test_nn_operations(self):
        ...
        xth = PTC.variable(val)
        xtf = KTF.variable(val)
        zth = PTC.eval(PTC.dropout(xth, level=0.2))
    ...
    def test_conv2d(self):
        ...
        for input_shape in [(2, 3, 4, 5), (2, 3, 5, 6)]:
            for kernel_shape in [(2, 2, 3, 4), (4, 3, 3, 4)]:
                for padding in ['valid', 'same']:
                    ...
                    kernel_th = PTC.variable(convert_kernel(kernel_val))
                    kernel_tf = KTF.variable(kernel_val)

                    zth = PTC.eval(PTC.conv2d(xth, kernel_th, data_format='channels_first'))
        ...
        zth = PTC.eval(PTC.conv2d(xth, kernel_th, data_format='channels_last'))
    ...
    def test_conv3d(self):
        ...
        for input_shape in [(2, 3, 4, 5, 4), (2, 3, 5, 4, 6)]:
            for kernel_shape in [(2, 2, 2, 3, 4), (3, 2, 4, 3, 4)]:
                ...
                zth = PTC.eval(PTC.conv3d(xth, kernel_th, data_format='channels_first'))
        ...
        zth = PTC.eval(PTC.conv3d(xth, kernel_th, data_format='channels_last'))
    ...
    def test_random_normal(self):
        ...
        rand = PTC.eval(PTC.random_normal((1000, 1000), mean=mean, stddev=std))
        assert rand.shape == (1000, 1000)
        assert np.abs(np.mean(rand) - mean) < 0.01
        assert np.abs(np.std(rand) - std) < 0.01
    ...
    def test_random_uniform(self):
        ...
        rand = PTC.eval(PTC.random_uniform((1000, 1000), min_val, max_val))
        assert rand.shape == (1000, 1000)
        assert np.abs(np.mean(rand)) < 0.01
        assert np.max(rand) <= max_val
        assert np.min(rand) >= min_val
    ...
    def test_random_binomial(self):
        ...
        rand = PTC.eval(PTC.random_binomial((1000, 1000), p))
        assert rand.shape == (1000, 1000)
        assert np.abs(np.mean(rand) - p) < 0.01
        assert np.max(rand) == 1
        assert np.min(rand) == 0
    ...
    def test_ctc(self):
        ...
        labels_th = PTC.variable(labels, dtype="int32")
        inputs_th = PTC.variable(inputs, dtype="float32")
        input_lens_th = PTC.variable(input_lens, dtype="int32")
        label_lens_th = PTC.variable(label_lens, dtype="int32")
        res = PTC.eval(PTC.ctc_batch_cost(labels_th, inputs_th, input_lens_th, label_lens_th))
    ...
    def test_arange(self):
        for test_value in (-20, 0, 1, 10):
            ...
            t_b = PTC.arange(test_value)
            b = PTC.eval(t_b)
            assert np.array_equal(b, np.arange(test_value))
            assert np.array_equal(a, b)
            assert KTF.dtype(t_a) == PTC.dtype(t_b)
        for start, stop, step in ((0, 5, 1), (-5, 5, 2), (0, 1, 2), (5, 0, -1)):
            a = KTF.eval(KTF.arange(start, stop, step))
            assert np.array_equal(a, np.arange(start, stop, step))
            b = PTC.eval(PTC.arange(start, stop, step))
            assert np.array_equal(b, np.arange(start, stop, step))
            assert np.array_equal(a, b)
        for dtype in ('int32', 'int64', 'float32', 'float64'):
            for backend in (KTF, PTC):
                t = backend.arange(10, dtype=dtype)
                assert backend.dtype(t) == dtype
        start, stop, step = 6.8, 2.3, -0.2
        a = KTF.eval(KTF.arange(start, stop, step, dtype='float32'))
        assert np.allclose(a, np.arange(start, stop, step))
        b = PTC.eval(PTC.arange(start, stop, step, dtype='float32'))
        assert np.allclose(b, np.arange(start, stop, step))
